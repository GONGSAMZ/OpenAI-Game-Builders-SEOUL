...
public static class SettingsPopupPrefabBuilder
{
    ...
    {
        Build<UI_SettingsOptions>("UI_SettingsOptions", true);
    ...
    static void Build<T>(string name, bool includeSlider) where T : UI_SettingsPopupBase
    {
        ...
        RectTransform pr = panel.rectTransform; pr.anchorMin = pr.anchorMax = new Vector2(.5f, .5f); pr.sizeDelta = new Vector2(760, includeSlider ? 450 : 520);
        ...
        if (includeSlider)
        {
            ...
            RectTransform sr = slider.GetComponent<RectTransform>(); sr.anchorMin = sr.anchorMax = new Vector2(.5f, .38f); sr.sizeDelta = new Vector2(560, 32);
